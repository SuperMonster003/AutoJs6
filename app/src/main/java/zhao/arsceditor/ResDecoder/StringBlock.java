...
public class StringBlock {
    ...
    public String getString(int index) throws CharacterCodingException {
        ...
        // 第 index 个字符串的长度
        int length;
        // 如果第 index 个字符串不是 UTF-8 型, 是 UTF-16 型
        if (!m_isUTF8) {
            // 获取该字符串的长度
            ...
            // 地址偏移 getVarint(this.m_strings, offset)[1]
        ...
        // 从 offest 地址处解析 length 长度个数据, 转化为 String 类型返回
        return decodeString(offset, length);
    }

    // 排序字符串, 由于字符串在 arsc 中是一一对应的, 所以不能改变原来的一一对应, 需要将列表进行排序
    public void sortStringBlock(String src, String tar) {
        // 查找原字符串在 strings 这个有序的数组中的位置, 以此确定修改后的字符串对应的位置
        int position = strings.indexOf(src);
        // 如果在 strings 中查找到这个字符串, 并且修改后的字符串不为空
        if (position >= 0 && !tar.equals("")) {
            // 从 strings 数组中移除原来的内容
    ...
    public void writeFully(LEDataOutputStream lmOut, ByteArrayOutputStream bOut) throws IOException {
        ...
        // 如果字符串样式偏移为 0
        if (stylesOffset == 0)
            // 获取新的 chunkSize, 经过试验发现, chunkSize= 原先的 chunkSize +
            // 修改后的字符串占用的字节 - 原来字符串占用的字节
            newChunkSize = newChunkSize + (bOut.size() - m_strings.length);
        else { // 如果字符串样式偏移不为 0
            // 获取新的 StylesOffset, 经过试验发现, StylesOffset= 原先的 chunkSize +
            // 修改后的字符串占用的字节 - 原来字符串占用的字节
            newChunkSize = newChunkSize + (bOut.size() - m_strings.length);
