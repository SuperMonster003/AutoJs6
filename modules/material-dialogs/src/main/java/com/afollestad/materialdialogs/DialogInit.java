...
class DialogInit {
    ...
    static void init(final MaterialDialog dialog) {
        ...
        if (builder.backgroundColor == 0) {
            builder.backgroundColor =
                    DialogUtils.resolveColor(
                            ...
                            DialogUtils.resolveColor(dialog.getContext(), android.R.attr.colorBackgroundFloating));
        }
        if (builder.backgroundColor != 0) {
        ...
        dialog.optionsView = dialog.view.findViewById(R.id.md_options);
        ...
        if (dialog.title != null) {
            ...
            if (builder.options != null) {
                dialog.optionsView.setVisibility(View.VISIBLE);
                dialog.optionsView.setOnClickListener(v -> {
                    var popupMenu = new PopupMenu(builder.context, v, Gravity.END);
                    popupMenu.getMenuInflater().inflate(R.menu.md_more_options, popupMenu.getMenu());
                    List<MaterialDialog.OptionMenuItemSpec> options = builder.options;
                    for (int i = 0; i < options.size(); i++) {
                        MaterialDialog.OptionMenuItemSpec option = options.get(i);
                        popupMenu.getMenu().add(Menu.NONE, getDialogMenuOptionsItemActionIdByIndex(i), i, option.title());
                    }
                    popupMenu.setOnMenuItemClickListener(menuItem -> {
                        List<MaterialDialog.OptionMenuItemSpec> optionMenuItemSpecs = builder.options;
                        for (int i = 0; i < optionMenuItemSpecs.size(); i++) {
                            MaterialDialog.OptionMenuItemSpec option = optionMenuItemSpecs.get(i);
                            int menuActionId = getDialogMenuOptionsItemActionIdByIndex(i);
                            if (menuItem.getItemId() == menuActionId) {
                                option.onClickListener().onClick(dialog);
                                return true;
                            }
                        }
                        return false;
            ...
            if (builder.title == null && builder.options == null) {
        ...
        if (dialog.checkBoxPrompt != null) {
            ...
            if (builder.widgetColorStateList != null) {
                MDTintHelper.setTint(dialog.checkBoxPrompt, builder.widgetColorStateList);
            } else {
    ...
    private static int getDialogMenuOptionsItemActionIdByIndex(int index) {
        if (index == 0) return R.id.menu_options_item_index_0;
        if (index == 1) return R.id.menu_options_item_index_1;
        if (index == 2) return R.id.menu_options_item_index_2;
        if (index == 3) return R.id.menu_options_item_index_3;
        if (index == 4) return R.id.menu_options_item_index_4;
        if (index == 5) return R.id.menu_options_item_index_5;
        if (index == 6) return R.id.menu_options_item_index_6;
        if (index == 7) return R.id.menu_options_item_index_7;
        if (index == 8) return R.id.menu_options_item_index_8;
        if (index == 9) return R.id.menu_options_item_index_9;
        if (index < 0) throw new IllegalArgumentException("Dialog menu options item index cannot be negative: " + index);
        throw new IllegalArgumentException("Dialog menu cannot have more than 10 options");
    }
    ...
    private static void setupInputDialog(final MaterialDialog dialog) {
        ...
        int widgetColor = dialog.builder.widgetColor;

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            Drawable textCursorDrawable = dialog.input.getTextCursorDrawable();
            textCursorDrawable.setTint(widgetColor);
            dialog.input.setTextCursorDrawable(textCursorDrawable);

            Drawable textSelectHandle = dialog.input.getTextSelectHandle();
            textSelectHandle.setTint(widgetColor);
            dialog.input.setTextSelectHandle(textSelectHandle);

            Drawable textSelectHandleLeft = dialog.input.getTextSelectHandleLeft();
            textSelectHandleLeft.setTint(widgetColor);
            dialog.input.setTextSelectHandleLeft(textSelectHandleLeft);

            Drawable textSelectHandleRight = dialog.input.getTextSelectHandleRight();
            textSelectHandleRight.setTint(widgetColor);
            dialog.input.setTextSelectHandleRight(textSelectHandleRight);
        }

        MDTintHelper.setTint(dialog.input, widgetColor);
